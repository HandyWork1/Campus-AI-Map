...
class Agent(Environment):
    ...
    def DriveAstar(graph, start, goal):
        ...
        while True:
            ...
            for i in neighbour.difference(p):  # go to all neighbours escept those visisted.
                ...
                h[i] = Agent.getH(i, destination) + Agent.getCost(l, Environment.DriveCost)  # calc hueristics
                print("hi",h)

            # best hueristic from list, then pick first item:
            sortedH = sorted(h.items(), key=operator.itemgetter(1))
            x = next(iter(sortedH[0]))  # contains the best hueristics
            p.append(x)
    ...
    def Astar(graph, start, goal):
        ...
        while True:
            ...
            for i in neighbour.difference(p):  # go to all neighbours escept those visisted.
                ...
                l.append(str(start))
                l.append(str(i))  # neighbour
                h[i] = Agent.getH(i, goal) + Agent.getCost(l, Environment.cost)  # calc hueristics
    ...
    def GBFS(graph, start, goal):
        ...
        while True:
            ...
            h = {}
            for i in neighbour.difference(p):  # go to all neighbours escept those visisted.
                h[i] = Agent.getH(i, goal)  # calc hueristics
    ...
    def displayGUI(myGraph, myDrive):
        ...
        root = Tk()  # Creating an object
        root.title('USIU-A MAP')
        root.geometry("800x500")  # User-interface size
        label0 = Label(root, text="Please select mode of travel: ")
        label0.grid(row=0, column=0)
        clicked0 = StringVar()
        clicked0.set("Walking")
        drop0 = OptionMenu(root, clicked0, *location0)
        drop0.grid(row=0, column=3, columnspan=2)

        def getDirection(root, location, myGraph, clicked):
            if (clicked0.get() == "Driving"):
                drop = OptionMenu(root, clicked, *location10)
                drop.grid(row=3, column=3, columnspan=2)
            else:
                drop = OptionMenu(root, clicked, *location)
                drop.grid(row=3, column=3, columnspan=2)
            label2 = Label(root, text="Please select destination: ")
            label2.grid(row=4, column=0)
            clicked2 = StringVar()
            clicked2.set("Gate_A")

            drop2 = OptionMenu(root, clicked2, *location)
            drop2.grid(row=4, column=3, columnspan=2)
            if(clicked0.get() == "MultiDestination"):
                label5 = Label(root, text="Enter the Third location")
                label5.grid(row=5, column=0)
                clicked3 = StringVar()
                clicked3.set("Gate_A")
                drop3 = OptionMenu(root, clicked3, *location)
                drop3.grid(row=5, column=3, columnspan=2)

            def results():
                if (clicked.get() == clicked2.get()):
                    msg.config(bg='red',text="Please do not enter the same location in both fields. \n")
                    ...
                    msg.config(text="Follow this route to get to your destination when driving: " + x + "\n")
                    if x != y:
                        msg2.config(bg='lightgreen',text="For an alternate route: " + y + "\n")
                elif(clicked0.get() == "Walking"):
                    x = str(Agent.Astar(Environment.myGraph, clicked.get(), clicked2.get()))
                    y = str(Agent.GBFS(Environment.myGraph, clicked.get(), clicked2.get()))
                    msg.config(bg='lightgreen',text="Follow this route to get to your destination when walking: " + x + "\n")
                    if x != y:
                        msg2.config(bg='lightgreen',text="For an alternate route: " + y + "\n")
                elif (clicked0.get() == "Driving"):
                    x=str(Agent.DriveAstar(Environment.Drive, clicked.get(), clicked2.get()))
                    msg.config(bg='lightgreen',text="Follow this route to get to your destination when driving: " + x + "\n")
                else:
                    msg.config(bg='lightgreen',text="Follow this route to get to your destination: " + str(
                        Agent.multiDest(Environment.myGraph, clicked.get(), clicked2.get(),clicked3.get())) + "\n")

            button = Button(root, text="Get Direction", padx= 10 , pady= 10, command=results).grid(row=6, column=3, columnspan=2)
            msg = Message(root, width=400, text=" \n")
            msg.grid(row=7, column=0, columnspan=5)
            msg2 = Message(root,width=400, text=" \n")
            msg2.grid(row=8, column=0, columnspan=5)
            Button(root, text="QUIT", padx= 25, pady= 10, fg="White", bg="Red", command=root.destroy).grid(row=10, column=0, columnspan=2)
            return

        def chooseDistance():
            label = Label(root, text="Please enter starting location: ")
            label.grid(row=3, column=0)
            clicked = StringVar()
            clicked.set("Gate_A")
            if (clicked0.get() == "Walking"):
                getDirection(root, location, myGraph, clicked)
            elif (clicked0.get() == "Parking For Disabled"):
                getDirection(root, location10, myDrive, clicked)
            elif(clicked0.get() == "Driving"):
                getDirection(root, location, myGraph, clicked)
            else:
                getDirection(root, location, myGraph, clicked)

        button10 = Button(root, text="Generate Distances",padx= 20 , pady= 10, command=chooseDistance).grid(row=2, column=3, columnspan=2)
        root.mainloop()
